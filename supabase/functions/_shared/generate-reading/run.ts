...
const LA_SO_CHI_TIET_PREVIEW_PROMPT_VER = "2026-05-29-preview-only";
...
## NỘI DUNG TỪNG MỤC
- menh_tong_quan (3–4 câu): Tổng quan lá số — Nhật Chủ, Mệnh, Dụng/Kỵ thần, cân bằng Ngũ Hành và nhịp đại vận hiện tại. Một đoạn mở đầu ấm, giúp người đọc hình dung bức tranh toàn cảnh trước các khía cạnh chi tiết.
...
## NỘI DUNG menh_tong_quan (3–4 câu)
Tổng quan lá số — Nhật Chủ, Mệnh, Dụng/Kỵ thần, cân bằng Ngũ Hành và nhịp đại vận hiện tại. Một đoạn mở đầu ấm, giúp người đọc hình dung bức tranh toàn cảnh. Không đi sâu tính cách / sự nghiệp / tình duyên (các chương khác sẽ có).
...
async function generateLaSoChiTietPreviewSections(
  ...
  if (!sections?.length) {
    const plain = await geminiReading(payload, 768, LA_SO_CHI_TIET_TIMEOUT_MS);
    const t = plain?.trim() ?? "";
    if (!t) return null;
    return [menhTongQuanFallbackSection(t)];
  }
  return laSoChiTietPreviewSections(sections);
}
