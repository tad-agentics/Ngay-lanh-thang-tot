...
- tieu-van, luu-nien: **dự phòng** — một đoạn 6–10 câu liền mạch khi hệ thống không dùng được định dạng 3 phần (JSON). Nội dung vẫn **bắt buộc nhất quán element_relation** (mục dưới).
...
const TIEU_VAN_LUU_NIEN_PROMPT_VER = "2026-03-28c";
...
/** tieu-van / luu-nien: đoạn một khối dự phòng. */
const TIEU_VAN_LUU_NIEN_TIMEOUT_MS = 14_000;
/** JSON 3 phần (tieu-van / luu-nien). */
const TIEU_VAN_LUU_NIEN_JSON_TIMEOUT_MS = 22_000;
const LA_SO_CHI_TIET_TIMEOUT_MS = 28_000;

const READING_MAX_TOKENS_DEFAULT = 512;
const READING_MAX_TOKENS_TIEU_VAN_LUU_NIEN = 1024;
const READING_MAX_TOKENS_TIEU_VAN_LUU_NIEN_JSON = 2048;
...
Ba khóa bắt buộc. Mỗi giá trị là một chuỗi tiếng Việt văn xuôi, **đúng 3–4 câu**, không gạch đầu dòng, không ký hiệu liệt kê:
- nhin_chung: nhịp tổng thể. Với tieu-van luôn diễn đạt **tháng**; với luu-nien luôn diễn đạt **năm**. Nếu data có trụ tháng / can chi thời gian, gợi nhẹ bối cảnh. Thể hiện quan hệ ngũ hành tháng hay năm với mệnh người dùng **khớp** element_relation hoặc elementRelation.
- thuc_tien: đời sống thực tế — công việc, tài chính, hợp tác, quan hệ. Bám các gợi ý có trong data (tags, linh_vuc, cac_giai, scores, details…) khi có; không tạo chi tiết không có trong data.
- ung_xu: nối với Đại vận hoặc cảnh báo nhịp dài nếu data có dai_van_context, can_luu hoặc tương đương. Cách ưu tiên việc, giữ nhịp, điều chỉnh kỳ vọng — vẫn nhất quán element_relation.
...
## GIỌNG VÀ ĐIỀU CẤM
Ấm áp, rõ ràng, xưng hô "bạn". Không emoji. Không markdown. Không phần trăm hay điểm số tự bịa. Không phán tuyệt đối ("chắc chắn", "không thể"). Không bịa ngoài dữ liệu. Không lặp tên field JSON hay chỉ số thô.`;

const TIEU_VAN_LUU_NIEN_JSON_RETRY =
  `Chỉ trả về một object JSON với đúng ba khóa: "nhin_chung", "thuc_tien", "ung_xu". Mỗi giá trị: string, 3–4 câu tiếng Việt văn xuôi, không markdown, không code fence.`;
