...
| Day luận follow-up today-only | `ask` returns `FOLLOW_UP_TODAY_ONLY` when `thread.day_iso` ≠ today (VN). Other days: anchor + read-only thread via `open`; max 10 FU on today. Shared `dayLuanFollowUpAllowed` in `_shared/day-luan-thread.ts`. Deploy: `functions deploy day-luan-chat` + FE. | No | Superseded | — |
| Tra cứu v2 | Inline flow `entry→thinking→results→day` on `/tra-cuu`; NLTT intro (`chon-ngay` LLM) + `reason_vi` rows; `top_n=15`; global daily quota `day_luan_daily_usage` (10/user/VN-day shared); hop-tuoi redirect; `/tra-cuu/ket-qua` hydrates flow. Deploy: `db push` + `functions deploy day-luan-chat` + FE. | No | Yes | — |
| Tra cứu results chat | Multi-turn AskBar on results: `tra-cuu-results-chat` Edge (DeepSeek v4 flash, thinking off) + LLM intent parse (`change_task` / `open_day` / `answer`); shares `day_luan_daily_usage` quota. Deploy: `db push` + `functions deploy tra-cuu-results-chat`. | No | Yes | — |
