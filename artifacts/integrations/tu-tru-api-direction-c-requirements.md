...
**OpenAPI hiện tại:** `https://tu-tru-api.fly.dev/openapi.json` (info.version **`0.1.2`**)  
**Consumer:** Ứng dụng **Ngày Lành Tháng Tốt** (Direction C) — proxy qua Supabase Edge `bat-tu`  
**Cập nhật:** 2026-05-29 (audit OpenAPI **0.1.2** — P2 schema ship; NLTT wire W1–W9 shipped) · 2026-05-29 (0.1.1) · 2026-05-28 (0.1.0)  
...
| **REQ-P2-05** | `GET /v1/phong-thuy?year=` — contract Phi Tinh + hướng/màu năm (màn 18 §04) | **P2** | `PhongThuyResponse` (`phi_tinh[]`, `huong_tot_nam_nay`, …) | Op có trong Edge; **chưa wire** màn 18 §04 | ✅ **Schema khóa** · 🔴 **NLTT wire phong-thuy** |
...
**Sau audit OpenAPI 0.1.2 (2026-05-29):** Upstream publish **58 schemas** — khóa thêm **`LaSoResponse`**, **`TuTruResponse`**, **`PhongThuyResponse`**, **`LichThangResponse`**, **`NgayHomNayResponse`**. NLTT đã wire **W1–W10** (local); còn upstream `{}` trên `tieu-van`, `hop-tuoi`, … — xem §11.
...
| GET | `/v1/phong-thuy` | `phong-thuy` | `/toi/luan-bat-tu` §04 | `PhongThuyResponse` — **NLTT chưa wire** màn 18 |
...
**Gap OpenAPI (0.1.2):** Schema đã khóa cho **day-detail**, **chon-ngay**, **luan-context**, **day-compare**, **luu-nien**, **la-so**, **tu-tru**, **phong-thuy**, **lich-thang**, **ngay-hom-nay**. Vẫn `{}` rỗng: **`tieu-van`**, **`hop-tuoi`**, `convert-date`, `profile`, `weekly-summary`. NLTT vẫn map linh hoạt (`la-so-ui.ts`, `home-bat-tu.ts`) cho field chưa align contract.
...
| **P2** | GET | `/v1/phong-thuy?year=` schema | Màn 18 §04 phong thủy | ✅ `PhongThuyResponse` · 🔴 NLTT wire |
