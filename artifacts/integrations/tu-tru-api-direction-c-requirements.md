...
**OpenAPI hiện tại:** [Swagger UI](https://tu-tru-api.fly.dev/docs#/) · `https://tu-tru-api.fly.dev/openapi.json` (info.version **`0.1.3`**)  
**Consumer:** Ứng dụng **Ngày Lành Tháng Tốt** (Direction C) — proxy qua Supabase Edge `bat-tu`  
**Cập nhật:** 2026-05-29 (audit OpenAPI **0.1.3** — `LuuNienResponse` / `PhongThuyResponse` / `personality_traits` khóa đủ màn 18) · 2026-05-29 (0.1.2) · 2026-05-28 (0.1.0)  
...
| ID | API phải làm | Ưu tiên | OpenAPI 0.1.2 | NLTT hiện tại | Trạng thái |
|----|----------------|---------|---------------|---------------|------------|
| **REQ-P0-01** | `day-detail` **không bắt buộc birth** khi xem lịch chung (anon `/ngay/:ngay`) | **P0** | `mode=generic\|personalized`; birth optional khi generic | `CDayDetailScreen` gửi `mode=generic` khi anon | ✅ **API + NLTT** |
| **REQ-P0-02** | `breakdown[]` **đúng 4 phần tử** + `reason_vi` cá nhân hoá + `sources[]` trên `day-detail` & `chon-ngay/detail` | **P0** | `BreakdownItem` + `DayDetailResponse` / `ChonNgayDetailResponse` | `day-luan-sectioned.ts` ưu tiên `reason_vi`; fallback heuristic còn khi thiếu | ✅ **Contract OK** · 🟡 **NLTT bỏ hết heuristic** |
| **REQ-P0-03** | `score_methodology` trên day-detail / ngay-hom-nay / lich-thang / chon-ngay | **P0** | ✅ Trên **day-detail**, **chon-ngay**, **chon-ngay/detail**, **ngay-hom-nay**, **lich-thang** | Collapsible đọc API trên day detail + tra cứu; `/lich` tháng vẫn copy tĩnh | ✅ **API shipped** · 🟡 **NLTT wire `/lich` tháng** |
| **REQ-P0-04** | `lich-thang` → `days[]` có `date`, `lunar_day`/`lunar_label`, `score`, `day_type` ổn định | **P0** | `LichThangResponse` + `LichThangDayItem` (17 field) | Mapper quét alias (`home-bat-tu.ts`) — có thể thu hẹp theo contract | ✅ **Schema khóa** · 🟡 **NLTT align mapper** |
| **REQ-P1-01** | Endpoint mới `GET /v1/day-detail/luan-context` | **P1** | `LuanContextResponse` (4 `breakdown_summary`, hints, followups) | Op `day-luan-context`; `generate-reading` detect `LuanContextResponse` | ✅ **API + NLTT** |
| **REQ-P1-02** | Endpoint mới `GET /v1/day-compare` | **P1** | `DayCompareResponse` | Op `day-compare`; chip 「So sánh với ngày mai」 deterministic | ✅ **API + NLTT** |
| **REQ-P1-03** | Cùng schema day-detail ↔ chon-ngay/detail | **P1** | `ChonNgayDetailResponse` mirror `DayDetailResponse` | ✅ **`parseDayDetailForView`** + alias `parseChonNgayDetailForView` | ✅ **Schema OK** · ✅ **NLTT W7** |
| **REQ-P1-04** | `POST /v1/chon-ngay` → **`ranked_days[]`** canonical + `empty_reason_vi` | **P1** | `ChonNgayResponse` + `RankedDay` | Ưu tiên `ranked_days`; hiển thị `empty_reason_vi`; fallback keys còn trong mapper | ✅ **API + NLTT** · 🟡 **bỏ fallback keys** |
| **REQ-P2-01** | `GET /v1/la-so` + **`POST /v1/tu-tru`** — contract UI màn 17 (pillars, ngũ hành, đại vận) | **P2** | `LaSoResponse` (26 field) · `TuTruResponse` (24 field) | Mapper `la-so-ui.ts` alias heuristic — **chưa align contract** | ✅ **Schema khóa** · 🔴 **NLTT W10** |
| **REQ-P2-01b** | `GET /v1/la-so` — facts cho Gemini **`la-so-chi-tiet`** (màn 18 interim MVP) | **P2** | `LaSoResponse` có `tinh_cach`…`tinh_duyen`, `dai_van_list` | FE ship: `la-so` → `generate-reading` `la-so-chi-tiet` | ✅ **Schema OK** · 🟡 **NLTT dùng field contract** |
| **REQ-P2-02** | `GET /v1/la-so/luu-nien?year=` — facts **vận năm** (≠ `tieu-van` vận tháng) | **P2** | `LuuNienResponse` (+ `engine_version`, `life_areas`, …) | Op `la-so-luu-nien`; `CBaziReadingScreen` wire `luu-nien` §03 | ✅ **API + NLTT** |
| **REQ-P2-03** | `POST /v1/tu-tru` → `engine_version` / `computed_at` | **P2** | `TuTruResponse.engine_version`, `computed_at` | G1 recompute có thể đọc stamp upstream | ✅ **Schema OK** · 🟡 **NLTT policy recompute** |
| **REQ-P2-04** | **`POST /v1/tu-tru` ≡ `GET /v1/la-so`** cùng birth — shape khớp cho cache `profiles.la_so` | **P2** | Hai schema **khác shape** (`TuTruMenhBasic` vs `PillarsContract` đầy đủ) | Onboarding lưu tu-tru; màn 17 cache, màn 18 live la-so | 🟡 **Schema riêng** · 🔴 **parity document / adapter** |
| **REQ-P2-05** | `GET /v1/phong-thuy?year=` — contract Phi Tinh + hướng/màu năm (màn 18 §04) | **P2** | `PhongThuyResponse` (`phi_tinh[]`, `huong_tot_nam_nay`, …) | Op `phong-thuy` + `loadBaziReadingFull` §04 | ✅ **Schema khóa** · ✅ **NLTT wire** · 🟡 rich UI khi runtime đủ field |
| **REQ-NLTT-01** | *(Không thuộc API)* tra cứu không trừ credit | — | — | ✅ Edge + FE shipped | ✅ **NLTT xong** |
| **REQ-P3-*** | Nice-to-have (semver, gio slots, purpose_rows, rate limit, …) | P3 | `GioSlot`, `PurposeRow` đã có schema | Workaround FE | ⚪ Backlog |

**Chú thích trạng thái:** 🔴 chưa có / chưa wire · 🟡 một phần · ✅ contract upstream OK · ⚪ backlog

**Thứ tự còn lại phía tu-tru-api:** P2-04 parity doc · schema **`tieu-van`**, **`hop-tuoi`** · fixture JSON đầy đủ (§7).

**Thứ tự ưu tiên phía NLTT:** Bỏ fallback heuristic còn lại khi upstream luôn trả đủ `breakdown[].reason_vi` — xem **§11.2**.
...
**Sau audit OpenAPI 0.1.3 (2026-05-29):** **62 schemas** — màn 18: **`LuuNienResponse`** (`life_areas`, `month_scores` / `month_score_values`, `warnings`, `quy_nhan`, `dai_van_next`, `teaser`), **`PhongThuyResponse`** (`phi_tinh[]` + `PhiTinhDirection`, `huong_tot_nam_nay`, `mau_may_man`, `phi_tinh_note_vi`), **`LaSoResponse.personality_traits[]`** cho §02. NLTT parsers + UI blocks đã có; còn wire **`dai_van_next`** §05 và **`personality_traits`** §02 trên FE.

**Sau audit OpenAPI 0.1.2 (2026-05-29):** Khóa **`LaSoResponse`**, **`TuTruResponse`**, **`PhongThuyResponse`**, **`LichThangResponse`**, **`NgayHomNayResponse`**. Còn upstream `{}` trên `tieu-van`, `hop-tuoi`, … — xem §11.

**Sau audit OpenAPI 0.1.1 (2026-05-29):** Team tu-tru-api ship **30 schemas** và P0/P1 (generic `day-detail`, `luan-context`, `day-compare`, `ranked_days`, `luu-nien`).

Tài liệu này liệt kê **việc upstream phải bàn giao** và **việc NLTT phải wire** — bổ sung OpenAPI 0.1.2, không thay thế docs hiện có.
...
| GET | `/v1/day-detail` | `day-detail` | `/ngay/:ngay`, `/luan-ai/day-*` | `mode=generic` cho anon — ✅ NLTT W4 |
| GET | `/v1/day-detail/luan-context` | `day-luan-context` | `generate-reading` anchor / chat ngày | ✅ NLTT W1–W2 |
| GET | `/v1/day-compare` | `day-compare` | Chip so sánh ngày trên `/luan-ai/day-*` | ✅ NLTT W3 |
...
| GET | `/v1/la-so/luu-nien` | `la-so-luu-nien` | `/toi/luan-bat-tu` §03 vận năm | ✅ NLTT W9 |
| GET | `/v1/tieu-van` | `tieu-van` | `/toi/luan-tieu-van` | **Vận tháng** — schema vẫn `{}` |
| POST | `/v1/hop-tuoi` | `hop-tuoi` | `/tra-cuu/hop-tuoi` | Schema vẫn `{}`; FE `source: tra_cuu` (REQ-NLTT-01) |
| GET | `/v1/phong-thuy` | `phong-thuy` | `/toi/luan-bat-tu` §04 | `PhongThuyResponse` — NLTT wire màn 18 |
...
**Gap OpenAPI (0.1.3):** Schema đã khóa cho **day-detail**, **chon-ngay**, **luan-context**, **day-compare**, **luu-nien**, **la-so**, **tu-tru**, **phong-thuy**, **lich-thang**, **ngay-hom-nay**. Vẫn `{}` rỗng: **`tieu-van`**, **`hop-tuoi`**, `convert-date`, `profile`, `weekly-summary`. NLTT map linh hoạt (`luu-nien-facts-ui.ts`, `phong-thuy-facts-ui.ts`, `la-so-ui.ts`) cho alias field.
...
**Vấn đề (0.1.0):** OpenAPI bắt buộc `birth_date=true`. Direction C cần `/ngay/:ngay` công khai (lịch chung + CTA đăng nhập).

**Trạng thái 0.1.1+:** ✅ Team API ship **phương án A** — query `mode=generic|personalized` (default `personalized`); `DayDetailResponse.personalized`, `breakdown_generic[]`. **NLTT W4:** ✅ `CDayDetailScreen` gửi `mode=generic` khi anon.
...
**NLTT hiện tại:** Một mapper (`parseDayDetailForView`); `chon-ngay-detail.ts` re-export alias.
...
| Priority | Method | Path / thay đổi | Blocker cho | OpenAPI 0.1.2 |
|----------|--------|-----------------|-------------|---------------|
| **P0** | GET | `day-detail?mode=generic` | Anon `/ngay/:ngay` | ✅ Shipped |
| **P0** | — | `breakdown` + `sources` + `score_methodology` | Màn 14–15, collapsible | ✅ `DayDetailResponse` |
| **P0** | — | `lich-thang` `days[].score`, `lunar_*` | `/lich/thang` | ✅ `LichThangResponse` · 🟡 NLTT align |
| **P1** | GET | `/v1/day-detail/luan-context` | Pipeline chat ngày | ✅ Shipped |
| **P1** | GET | `/v1/day-compare` | Chip so sánh ngày | ✅ Shipped |
| **P1** | — | `chon-ngay` → `ranked_days[]` + `empty_reason_vi` | `/tra-cuu/ket-qua` | ✅ `ChonNgayResponse` |
| **P1** | — | Unified schema day-detail ↔ chon-ngay/detail | Một mapper FE | ✅ Schemas mirror |
| **P2** | GET | `/v1/la-so` OpenAPI đầy đủ | Màn 17 + `la-so-chi-tiet` | ✅ `LaSoResponse` · 🔴 NLTT W10 |
| **P2** | — | `tu-tru` ≡ `la-so` parity | Cache `profiles.la_so` | 🟡 `TuTruResponse` khác shape |
| **P2** | GET | `/v1/la-so/luu-nien?year=` | Màn 18 §03 vận năm | ✅ `LuuNienResponse` + NLTT W9 |
| **P2** | GET | `/v1/phong-thuy?year=` schema | Màn 18 §04 phong thủy | ✅ `PhongThuyResponse` · ✅ NLTT wire · 🟡 rich UI runtime |
| **P2** | — | `tu-tru` version stamp | G1 recompute | ✅ `TuTruResponse.engine_version` |
| **P3** | — | OpenAPI schemas đầy đủ (không `{}`) | QA contract | 🟡 5 endpoint còn `{}` |
...
| Schema `luan-context` + `day-compare` facts | ✅ REQ-P1-01/02 | ✅ NLTT wire W1–W3 |
| Compact builder interim từ raw `day-detail` | ❌ | ✅ `_shared/day-luan-prompt-context.ts` (**tạm — bỏ sau wire P1-01**) |
...
| Schema `luu-nien` facts vận **năm** | ✅ REQ-P2-02 | ✅ NLTT W9 — `CBaziReadingScreen` |
...
|---------------|------------------------|-------------------|
| `day-detail?mode=generic` | `day-detail` (query `mode=generic`) | `CDayDetailScreen` anon — **W4** |
| `luan-context` | `day-luan-context` *(mới)* | `generate-reading` + bỏ interim builder — **W1–W2** |
| `day-compare` | `day-compare` *(mới)* | `CAiTypedScreen` chip — **W3** |
...
| `la-so/luu-nien?year=` | `la-so-luu-nien` *(mới)* | Màn 18 §03 — **W9** |
...
| Tuần | Deliverable API | Trạng thái (2026-05-29) |
|------|-----------------|-------------------------|
| T0 | Xác nhận REQ-P0-01..04 + sample JSON | ✅ Contract P0; fixtures `la-so`/`tu-tru` còn thiếu |
| T1 | Ship P0 lên staging | ✅ Production Fly 0.1.2 |
| T2 | P1 `ranked_days[]` + unified day-detail schema | ✅ OpenAPI + NLTT wire |
| T2 | P1 `luan-context` + `day-compare` | ✅ OpenAPI + NLTT wire |
| T3 | P2 `la-so` + `tu-tru` + `phong-thuy?year=` OpenAPI | ✅ Schema 0.1.2; 🟡 parity tu-tru↔la-so; NLTT W10 |
| — | NLTT REQ-NLTT-01 billing bypass | ✅ Shipped |
| **T+** | **NLTT wire backlog (§11.2)** | ✅ W1–W10 shipped (local) |

**Owner phía NLTT:** Tech Lead — QA parity runtime `la-so`/`tu-tru`; upstream schemas còn `{}`.

---

## 11. Audit OpenAPI 0.1.1 (2026-05-29)

**Nguồn:** [Swagger UI](https://tu-tru-api.fly.dev/docs#/) · [openapi.json](https://tu-tru-api.fly.dev/openapi.json) · spot-check runtime 2026-05-29.

### 11.1 Thay đổi so với 0.1.0

| Hạng mục | 0.1.0 | 0.1.1 |
|----------|-------|-------|
| `info.version` | `0.1.0` | **`0.1.1`** |
| Số path | 17 | **19** |
| Component schemas | ~0 (response `{}`) | **30** named models |
| Endpoint mới | — | `GET /v1/day-detail/luan-context`, `GET /v1/day-compare` |

**Schemas mới đáng chú ý:** `DayDetailResponse`, `BreakdownItem`, `ScoreMethodology`, `ChonNgayResponse`, `RankedDay`, `ChonNgayDetailResponse`, `LuanContextResponse`, `DayCompareResponse`, `LuuNienResponse`, `GioSlot`, `SourceRef`, `PurposeRow`.

**Endpoint vẫn response `{}`:** `ngay-hom-nay`, `lich-thang`, `la-so`, `tu-tru`, `tieu-van`, `hop-tuoi`, `phong-thuy`, `convert-date`, `profile`, `weekly-summary`.

### 11.1.1 Runtime spot-check (mẫu)

| Call | Kết quả |
|------|---------|
| `GET /v1/day-detail?date=2026-05-29&mode=generic` | `personalized: false`, `score: 85`, **4** breakdown ids (`truc`, `sao28`, `can_chi_laso`, `gio_vang`), có `score_methodology.summary_vi` |
| `GET /v1/day-detail/luan-context?date=…&birth_date=…` | **4** `breakdown_summary`, `anchor_question_hint_vi`, `suggested_followups[]` |
| `GET /v1/day-compare?date_a=…&date_b=…` | `comparison_vi`, `delta_score`, `better_for[]`, `sources[]` |

**Ghi chú upstream:** Generic mode trả breakdown id `can_chi_laso` dù `personalized: false` — xác nhận naming có cố ý hay cần alias generic.

### 11.2 NLTT wire backlog (ưu tiên)

| # | Task | REQ | Trạng thái | Ghi chú |
|---|------|-----|------------|---------|
| W1 | Edge `bat-tu` op **`day-luan-context`** | P1-01 | ✅ Shipped | `bat-tu` op + hook parallel fetch |
| W2 | `generate-reading` dùng **`luan-context`** | P1-01 | ✅ Shipped | `isLuanContextPayload()`; interim builder fallback |
| W3 | Edge op **`day-compare`** + chip `CAiTypedScreen` | P1-02 | ✅ Shipped | Chip deterministic, không LLM |
| W4 | Anon `/ngay/:ngay` **`mode=generic`** | P0-01 | ✅ Shipped | `CDayDetailScreen` |
| W5 | Collapsible **`score_methodology`** từ API | P0-03 | ✅ Shipped | Day detail + tra cứu + `/lich` + `/lich/thang` |
| W6 | Tra cứu **`ranked_days[]`** + `empty_reason_vi` | P1-04 | ✅ Shipped | Chỉ đọc `ranked_days[]`; bỏ legacy array keys |
| W7 | Hợp nhất mapper day-detail ↔ chon-ngay | P1-03 | ✅ Shipped | `parseChonNgayDetailForView` alias; `sourceLabels` trên panel |
| W8 | Sectioned panel **1:1** `breakdown[].reason_vi` | P0-02 | ✅ Shipped | Heuristic body khi thiếu `reason_vi` |
| W9 | **`la-so-luu-nien`** + màn 18 §03 | P2-02 | ✅ Shipped | `CBaziReadingScreen` + `luu-nien-ui.ts` |
| W10 | Align **`la-so` / `tu-tru` / `phong-thuy`** contract | P2-01–05 | ✅ Shipped | `la-so-normalize`, phong-thuy §04, methodology `/lich` |

**Wave tiếp theo:** Bỏ heuristic body fallback khi upstream đủ `reason_vi`; upstream publish `tieu-van` / `hop-tuoi` schemas.

### 11.3 Còn thiếu phía tu-tru-api (post-0.1.2)

1. Publish schema **`tieu-van`** (vận tháng) — vẫn `{}`.
2. Publish schema **`hop-tuoi`** — vẫn `{}`.
3. Document / adapter **parity `tu-tru` ↔ `la-so`** (REQ-P2-04) — hai schema khác shape.
4. Fixture JSON đầy đủ theo §7 (`LaSoResponse`, `TuTruResponse`, `PhongThuyResponse` mẫu).
5. (Tuỳ chọn) `minItems: 4` trên `breakdown` trong OpenAPI.

### 11.4 Audit OpenAPI 0.1.2 (2026-05-29)

**Nguồn:** [Swagger UI](https://tu-tru-api.fly.dev/docs#/) · [openapi.json](https://tu-tru-api.fly.dev/openapi.json)

| Hạng mục | 0.1.1 | 0.1.2 |
|----------|-------|-------|
| `info.version` | `0.1.1` | **`0.1.2`** |
| Số path | 19 | **19** (giữ nguyên) |
| Component schemas | 30 | **58** (+28) |
| Endpoint mới | `luan-context`, `day-compare` | — |

**Schemas mới khóa (từ `{}`):**

| Schema | Endpoint | Field đáng chú ý cho NLTT |
|--------|----------|---------------------------|
| `LaSoResponse` | `GET /v1/la-so` | `pillars`, `nhat_chu`, `dung_than`/`ky_than`, `dai_van_list`, `element_counts`, `tinh_cach`…`tinh_duyen`, `engine_version` |
| `TuTruResponse` | `POST /v1/tu-tru` | `engine_version`, `computed_at`, `birth_year_can_chi`, `menh` (`TuTruMenhBasic`), `dai_van_list` |
| `PhongThuyResponse` | `GET /v1/phong-thuy` | `phi_tinh[]`, `huong_tot_nam_nay`, `huong_xau_nam_nay`, `mau_may_man`, `hoa_giai[]` |
| `LichThangResponse` | `GET /v1/lich-thang` | `days[]` (`LichThangDayItem`), **`score_methodology`** |
| `NgayHomNayResponse` | `GET /v1/ngay-hom-nay` | **`score_methodology`**, `daily_advice`, `bat_tu` |

**Endpoint vẫn response `{}`:** `tieu-van`, `hop-tuoi`, `convert-date`, `profile` (POST/GET), `weekly-summary`.

**NLTT follow-up sau 0.1.2:**

- **`la-so-ui.ts`:** map theo `LaSoResponse` / `PillarsContract` — bỏ alias heuristic khi QA parity runtime.
- **`CBaziReadingScreen` §04:** wire `phong-thuy` + optional Gemini prose.
- **`home-bat-tu.ts` / `CMonthScreen`:** đọc `score_methodology` từ `lich-thang` / `ngay-hom-nay`.
