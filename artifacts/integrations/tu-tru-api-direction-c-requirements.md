...
**OpenAPI hiện tại:** `https://tu-tru-api.fly.dev/openapi.json` (info.version **`0.1.1`**)  
**Consumer:** Ứng dụng **Ngày Lành Tháng Tốt** (Direction C) — proxy qua Supabase Edge `bat-tu`  
**Cập nhật:** 2026-05-29 (audit OpenAPI **0.1.1** — P0/P1 contract ship; NLTT wire backlog mở) · 2026-05-28 (baseline 0.1.0)  
...
| ID | API phải làm | Ưu tiên | OpenAPI 0.1.1 | NLTT hiện tại | Trạng thái |
|----|----------------|---------|---------------|---------------|------------|
| **REQ-P0-01** | `day-detail` **không bắt buộc birth** khi xem lịch chung (anon `/ngay/:ngay`) | **P0** | `mode=generic\|personalized`; birth optional khi generic | FE gửi `{ date }` only; **chưa** gửi `mode=generic` | ✅ **API shipped** · 🔴 **NLTT chưa wire** |
| **REQ-P0-02** | `breakdown[]` **đúng 4 phần tử** + `reason_vi` cá nhân hoá + `sources[]` trên `day-detail` & `chon-ngay/detail` | **P0** | `BreakdownItem` + `DayDetailResponse` / `ChonNgayDetailResponse` | Map `breakdown`; vẫn fallback heuristic body khi thiếu prose | ✅ **Contract OK** · 🟡 **NLTT bỏ heuristic khi wire** |
| **REQ-P0-03** | `score_methodology` trên day-detail / ngay-hom-nay / lich-thang / chon-ngay | **P0** | Có trên `day-detail`, `chon-ngay`, `chon-ngay/detail` — **chưa** trên `ngay-hom-nay` / `lich-thang` | Copy tĩnh FE (`DayScoreMethodologyCollapsible`, `TraCuuMethodologyCollapsible`) | 🟡 **Một phần API** · 🔴 **NLTT chưa đọc API** |
| **REQ-P0-04** | `lich-thang` → `days[]` có `date`, `lunar_day`/`lunar_label`, `score`, `day_type` ổn định | **P0** | Response vẫn `{}` | Mapper quét ~10 key (`home-bat-tu.ts`) | 🟡 **Runtime có thể OK; schema chưa khóa** |
| **REQ-P1-01** | Endpoint mới `GET /v1/day-detail/luan-context` | **P1** | `LuanContextResponse` (4 `breakdown_summary`, hints, followups) | Interim: `buildDayLuanPromptContext()` + raw `day-detail` anchor | ✅ **API shipped** · 🔴 **NLTT chưa wire** |
| **REQ-P1-02** | Endpoint mới `GET /v1/day-compare` | **P1** | `DayCompareResponse` | Chip 「So sánh với ngày mai」 — copy tĩnh / LLM | ✅ **API shipped** · 🔴 **NLTT chưa wire** |
| **REQ-P1-03** | Cùng schema day-detail ↔ chon-ngay/detail | **P1** | `ChonNgayDetailResponse` mirror `DayDetailResponse` | Hai mapper riêng (`day-detail-view`, `chon-ngay-detail`) | ✅ **Schema OK** · 🟡 **NLTT hợp nhất mapper** |
| **REQ-P1-04** | `POST /v1/chon-ngay` → **`ranked_days[]`** canonical + `empty_reason_vi` | **P1** | `ChonNgayResponse` + `RankedDay` | Ưu tiên `ranked_days`, fallback 11 array keys | ✅ **Schema OK** · 🟡 **NLTT bỏ fallback keys** |
| **REQ-P2-01** | `GET /v1/la-so` + **`POST /v1/tu-tru`** — contract UI màn 17 (pillars, ngũ hành, đại vận) | **P2** | `{}` rỗng | Mapper `la-so-ui.ts` đọc nhiều alias từ `profiles.la_so` | 🔴 **Chưa khóa OpenAPI** |
| **REQ-P2-01b** | `GET /v1/la-so` — facts cho Gemini **`la-so-chi-tiet`** (màn 18 interim MVP) | **P2** | `{}` rỗng | FE ship interim: `la-so` → `generate-reading` `la-so-chi-tiet` (5 aspect) | 🟡 **Interim NLTT** · 🔴 **Schema thiếu** |
| **REQ-P2-02** | `GET /v1/la-so/luu-nien?year=` — facts **vận năm** (≠ `tieu-van` vận tháng) | **P2** | `LuuNienResponse` (+ `engine_version`, `life_areas`, …) | Edge có prompt `luu-nien`; FE màn 18 **chưa gọi** | ✅ **API shipped** · 🔴 **NLTT chưa wire** |
| **REQ-P2-03** | `POST /v1/tu-tru` → `engine_version` / `computed_at` | **P2** | `tu-tru` response `{}`; stamp có trên `luu-nien` | G1 recompute dựa policy NLTT, chưa có stamp upstream | 🟡 **Một phần** (`luu-nien` only) |
...
| **REQ-P2-05** | `GET /v1/phong-thuy?year=` — contract Phi Tinh + hướng/màu năm (màn 18 §04) | **P2** | `{}` rỗng | Op có trong Edge; chưa wire màn 18 | 🔴 **Chưa khóa OpenAPI** |
...
**Thứ tự còn lại phía tu-tru-api:** P0-04 schema · P0-03 mở rộng `ngay-hom-nay`/`lich-thang` · P2 `la-so`/`tu-tru`/`phong-thuy` OpenAPI.

**Thứ tự ưu tiên phía NLTT (wire):** xem **§11.2**.
...
**Sau audit OpenAPI 0.1.1 (2026-05-29):** Team tu-tru-api đã publish **30 schemas** và ship phần lớn **P0/P1** (generic `day-detail`, `breakdown`, `luan-context`, `day-compare`, `ranked_days`, `luu-nien`). NLTT chuyển sang giai đoạn **wire + bỏ interim** — xem §11.

Tài liệu này liệt kê **việc upstream phải bàn giao** và **việc NLTT phải wire** — bổ sung OpenAPI 0.1.1, không thay thế docs hiện có.
...
| GET | `/v1/day-detail` | `day-detail` | `/ngay/:ngay`, `/luan-ai/day-*` | `mode=generic` cho anon (P0-01) — **NLTT chưa gửi `mode`** |
| GET | `/v1/day-detail/luan-context` | *(chưa có op)* | `generate-reading` anchor / chat ngày | **P1-01 shipped** — NLTT wire `day-luan-context` |
| GET | `/v1/day-compare` | *(chưa có op)* | Chip so sánh ngày trên `/luan-ai/day-*` | **P1-02 shipped** — NLTT wire `day-compare` |
...
| GET | `/v1/la-so/luu-nien` | *(chưa có op)* | `/toi/luan-bat-tu` §03 vận năm *(Make)* | **P2-02 shipped** (`LuuNienResponse`) — NLTT wire |
...
**Gap OpenAPI (0.1.1):** Schema đã khóa cho **day-detail**, **chon-ngay**, **luan-context**, **day-compare**, **luu-nien**. Vẫn `{}` rỗng: `ngay-hom-nay`, `lich-thang`, `la-so`, `tu-tru`, `tieu-van`, `hop-tuoi`, `phong-thuy`. NLTT vẫn map linh hoạt (`day-detail-view.ts`, `home-bat-tu.ts`, `chon-ngay-result.ts`, `la-so-ui.ts`) cho các endpoint chưa khóa.
...
**Trạng thái 0.1.1:** ✅ Team API ship **phương án A** — query `mode=generic|personalized` (default `personalized`); `DayDetailResponse.personalized`, `breakdown_generic[]`. **NLTT W4:** Edge + FE chưa gửi `mode=generic`.
...
| Priority | Method | Path / thay đổi | Blocker cho | OpenAPI 0.1.1 |
|----------|--------|-----------------|-------------|---------------|
...
| **P0** | — | `lich-thang` `days[].score`, `lunar_*` | `/lich/thang` | 🟡 Runtime; schema `{}` |
...
| **P2** | GET | `/v1/la-so` OpenAPI đầy đủ | Màn 17 + `la-so-chi-tiet` | 🔴 `{}` |
| **P2** | — | `tu-tru` ≡ `la-so` parity | Cache `profiles.la_so` | 🔴 `{}` |
| **P2** | GET | `/v1/la-so/luu-nien?year=` | Màn 18 §03 vận năm | ✅ `LuuNienResponse` |
| **P2** | GET | `/v1/phong-thuy?year=` schema | Màn 18 §04 phong thủy | 🔴 `{}` |
| **P2** | — | `tu-tru` version stamp | G1 recompute | 🟡 Có trên `luu-nien` only |
| **P3** | — | OpenAPI schemas đầy đủ (không `{}`) | QA contract | 🟡 7 endpoint còn `{}` |
...
| Schema `luan-context` + `day-compare` facts | ✅ REQ-P1-01/02 | Consume qua `bat-tu` — **🔴 chưa wire** |
...
| Schema `luu-nien` facts vận **năm** | ✅ REQ-P2-02 | `generate-reading` `luu-nien` — **🔴 màn 18 chưa gọi** |
...
|------|-----------------|-------------------------|
| T0 | Xác nhận REQ-P0-01..04 + sample JSON | ✅ P0-01/02 contract; fixtures còn thiếu `la-so` |
| T1 | Ship P0 lên staging | ✅ Production Fly 0.1.1 |
| T2 | P1 `ranked_days[]` + unified day-detail schema | ✅ OpenAPI; 🟡 NLTT wire FE |
| T2 | P1 `luan-context` + `day-compare` | ✅ OpenAPI + runtime; 🔴 NLTT wire |
| T3 | P2 `la-so` + parity `tu-tru` + `phong-thuy?year=` OpenAPI | 🟡 `luu-nien` ✅; còn lại 🔴 |
| — | NLTT REQ-NLTT-01 billing bypass | ✅ Shipped |
| **T+** | **NLTT wire backlog (§11.2)** | 🔴 Chưa bắt đầu |

**Owner phía NLTT:** Tech Lead — wire `bat-tu` ops + bỏ interim mappers, QA regression W4–W6.
...
| # | Task | REQ | Files / ops dự kiến | Gate |
|---|------|-----|----------------------|------|
| W1 | Edge `bat-tu` op **`day-luan-context`** → proxy `/v1/day-detail/luan-context` | P1-01 | `supabase/functions/bat-tu/index.ts` | QA anchor luận |
| W2 | `generate-reading` dùng **`luan-context`** thay `buildDayLuanPromptContext()` | P1-01 | `generate-reading/index.ts`, xóa interim `_shared/day-luan-prompt-context.ts` | Luận anchor khớp facts API |
| W3 | Edge op **`day-compare`** + chip follow-up trên `CAiTypedScreen` | P1-02 | `bat-tu`, `CAiTypedScreen.tsx`, `useDayLuanReading.ts` | Chip 「So sánh với ngày mai」 deterministic |
| W4 | Anon `/ngay/:ngay` gửi **`mode=generic`** | P0-01 | `bat-tu` `day-detail`, `CDayDetailScreen.tsx` | Không 400 khi thiếu birth |
| W5 | Collapsible methodology đọc **`score_methodology`** từ API | P0-03 | `DayScoreMethodologyCollapsible`, `TraCuuMethodologyCollapsible` | Bỏ copy tĩnh |
| W6 | Tra cứu **`ranked_days[]` only** + `empty_reason_vi` | P1-04 | `chon-ngay-result.ts`, `tra-cuu.ket-qua.tsx` | Bỏ 11 fallback array keys |
| W7 | Hợp nhất mapper **`day-detail-view`** ↔ **`chon-ngay-detail`** | P1-03 | `app/lib/day-detail-view.ts` | Một code path breakdown UI |
| W8 | `buildDayLuanSectionRows()` map **1:1** `breakdown[].reason_vi` — bỏ heuristic body | P0-02 | `day-luan-sectioned.ts` | Màn 14–15 sectioned panel |
| W9 | Edge op **`la-so-luu-nien`** + màn 18 §03 | P2-02 | `bat-tu`, `CBaziReadingScreen.tsx`, `generate-reading` | Vận năm live |
| W10 | Chờ upstream P2 rồi wire **`la-so` / `tu-tru` / `phong-thuy`** OpenAPI | P2-01–05 | `la-so-ui.ts` | Bỏ alias heuristic |

**Wave đề xuất:** W1–W4 (daily loop + luận) → W5–W8 (methodology + tra cứu + breakdown) → W9–W10 (lá số / vận năm).

### 11.3 Còn thiếu phía tu-tru-api (post-0.1.1)

1. Publish schema **`ngay-hom-nay`**, **`lich-thang`** (gồm `days[]`, `score_methodology` nếu có).
2. Publish schema **`la-so`**, **`tu-tru`**, **`phong-thuy`** (P2 blocker màn 17–18).
3. (Tuỳ chọn) `minItems: 4` trên `breakdown` trong OpenAPI.
4. Fixture JSON đầy đủ theo §7 (đặc biệt `la-so` / `tu-tru` parity).
