...
// ═══════════════════════════════════════════════════════════════════
function CAITyped() {
  // Voice answer first (typed), then sectioned breakdown reveals progressively
  const fullText = "Hôm nay ngày Mậu Tuất — Thổ. Mệnh bạn là Quý Thủy, Thổ khắc Thủy, lẽ ra phải xấu. Nhưng giờ Thìn buổi sáng có Mộc khí vượng — Mộc khắc Thổ, hoá giải được. Vì vậy buổi sáng tốt cho ký kết, nhưng sang chiều khi Mộc khí lui, Thổ vượng trở lại — không nên động thổ.";
  const [n, setN] = cgUseState(0);
  cgUseEffect(() => {
  ...
  const sections = [
  { title: 'Trực ngày', verdict: 'Định', body: 'Trực Định — vững vàng, định đoạt. Hợp ký kết, định ước, an cư. Tránh khởi sự lớn vì Định khó chuyển.', src: '[1]', score: '+24' },
  { title: 'Nhị thập bát tú', verdict: 'Sao Thiên Đức', body: 'Thiên Đức tinh đáo nhật — phúc đức tinh chiếu. Hộ trì việc thiện, đặc biệt hợp khai trương, cưới hỏi.', src: '[2]', score: '+20' },
  { title: 'Can chi · tương sinh với lá số bạn', verdict: 'Mậu Tuất → Quý Thủy', body: 'Mậu Thổ khắc Quý Thủy — về cơ bản kỵ. Nhưng trong tứ trụ bạn có trụ Nhật Quý Tỵ, Tỵ là Hỏa sinh Thổ — quan hệ phức tạp, không phải xung mạnh.', src: '[3]', score: '+18' },
  { title: 'Giờ vàng trong ngày', verdict: 'Thìn 7–9h · Mùi 13–15h', body: 'Thìn là Mộc-Thổ, Mùi là Thổ. Buổi sáng Mộc khí át Thổ → hợp Quý Thủy của bạn. Đáng tránh giờ Tỵ và Ngọ.', src: '[4]', score: '+14' }];


  return (
    <div style={{ width: 390, height: 800, background: CT.paper, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      <StatusBar />
      <CBackBar title="Luận giải · ngày 26.05" right={<Mono style={{ color: CT.muted, fontSize: 9 }}>AI · có nguồn</Mono>} />
      ...
      <div style={{ flex: 1, padding: '8px 24px 24px', overflow: 'auto' }}>
        ...
        <div style={{ marginTop: 22, display: 'flex', gap: 12, alignItems: 'flex-start' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <p style={{ marginTop: 6, marginBottom: 0, fontFamily: 'var(--serif)', fontSize: 14, color: CT.ink, lineHeight: 1.65 }}>
              {fullText.slice(0, n)}<span style={{ display: 'inline-block', width: 7, height: 14, background: CT.ink, marginLeft: 2, verticalAlign: 'middle', opacity: done ? 0 : 1, animation: 'b-cursor-blink 1s steps(2) infinite' }} />
            </p>
            <style>{`@keyframes b-cursor-blink { 50% { opacity: 0; } }`}</style>
          </div>
        </div>

        {/* Sectioned breakdown — was screen 16, now revealed inline after typing completes */}
        {done &&
        <div style={{ marginTop: 26, paddingTop: 18, borderTop: `1px solid ${CT.hairline}`, animation: 'b-fade-in 320ms ease-out' }}>
            <style>{`@keyframes b-fade-in { from { opacity: 0; transform: translateY(4px); } to { opacity: 1; transform: none; } }`}</style>
            <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
              <Mono style={{ color: CT.goldDeep, fontSize: 10 }}>Phân tích chi tiết · 4 yếu tố</Mono>
              <span style={{ fontFamily: 'var(--serif)', fontSize: 11.5, color: CT.muted }}>26.05.2026 · ngày Mậu Tuất</span>
            </div>

            {sections.map((s, i) =>
          <div key={i} style={{ marginTop: 16, paddingTop: 14, borderTop: i === 0 ? 'none' : `1px solid ${CT.hairline2}` }}>
                <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
                ...
                <p style={{ marginTop: 6, marginBottom: 0, fontFamily: 'var(--serif)', fontSize: 13, color: CT.ink2, lineHeight: 1.6 }}>
            ...
            {/* Total */}
            <div style={{ marginTop: 20, padding: '14px 0', borderTop: `2px solid ${CT.ink}`, display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
            ...
            <div style={{ marginTop: 18 }}>
              ...
              map(([num, t]) =>
              <div key={num} style={{ display: 'flex', gap: 8, fontFamily: 'var(--serif)', fontSize: 12, color: CT.ink2, lineHeight: 1.45 }}>
                    <span style={{ color: CT.goldDeep, fontFamily: 'var(--mono)', fontSize: 10, minWidth: 24 }}>{num}</span>
                    <span>{t}</span>
                  </div>
              )}
              </div>
            ...
            {/* ─── Follow-up turn — demonstrates thread pattern ─── */}
            <div style={{ marginTop: 22, paddingTop: 18, borderTop: `1px solid ${CT.hairline}` }}>
              {/* Q2 — user-asked follow-up */}
              <div style={{ padding: '10px 14px', background: 'rgba(154,124,34,0.06)', borderLeft: `2px solid ${CT.goldDeep}` }}>
                <Mono style={{ color: CT.goldDeep, fontSize: 9 }}>Bạn hỏi</Mono>
                <div style={{ marginTop: 3, fontFamily: 'var(--serif)', fontStyle: 'italic', fontSize: 13, color: CT.ink2, lineHeight: 1.5 }}>
                  "Giờ nào trong ngày tốt nhất?"
                </div>
              </div>
              {/* A2 — voice-only response (no sectioned re-analysis; already shown for the anchor question) */}
              <div style={{ marginTop: 14, display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <Mono style={{ color: CT.muted, fontSize: 9 }}>NLTT luận</Mono>
                  <p style={{ marginTop: 4, marginBottom: 0, fontFamily: 'var(--serif)', fontSize: 13.5, color: CT.ink, lineHeight: 1.6 }}>
                    Giờ Thìn (7–9h sáng) là tốt nhất — Mộc khí vượng tối đa, hoá giải Thổ khắc Thuỷ. Giờ Mùi (13–15h) là phương án dự phòng nhưng đã yếu hơn. Tránh giờ Tỵ và Ngọ vì Hoả sinh Thổ, làm tăng khắc với mệnh bạn. <span style={{ color: CT.goldDeep, fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer' }}>[4]</span>
                  </p>
                </div>
              </div>
            </div>

            {/* Remaining suggested chips — 1 đã dùng, còn 2 */}
            <div style={{ marginTop: 22, paddingTop: 16, borderTop: `1px solid ${CT.hairline}` }}>
              <Mono style={{ color: CT.muted, fontSize: 9 }}>Hỏi tiếp gợi ý</Mono>
              <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 6 }}>
                {['Hôm nay có nên ký hợp đồng không?', 'So sánh với ngày mai'].map((q) =>
      ...
      {/* Bottom input bar — scope-locked, rate-limited */}
      <div style={{ padding: '8px 20px 18px', background: CT.paper, borderTop: `1px solid ${CT.hairline}` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: '#fff', border: `1px solid ${CT.hairline}`, borderRadius: 999 }}>
          <input type="text" placeholder="Hỏi tiếp về ngày 26.05…" style={{ flex: 1, border: 'none', outline: 'none', background: 'transparent', fontFamily: 'var(--serif)', fontSize: 13, color: CT.ink }} />
        ...
        {/* Scope + remaining quota — communicates "không phải freeform ChatGPT" */}
        <div style={{ marginTop: 7, padding: '0 4px', display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 8 }}>
          <span style={{ fontFamily: 'var(--serif)', fontSize: 10.5, color: CT.muted, fontStyle: 'italic' }}>
            Chỉ trả lời về ngày này và lá số của bạn
          </span>
          <span style={{ fontFamily: 'var(--mono)', fontSize: 9, color: CT.muted, letterSpacing: '0.08em', textTransform: 'uppercase', fontVariantNumeric: 'tabular-nums' }}>
            còn <span style={{ color: CT.goldDeep, fontWeight: 700 }}>8/10</span> câu hôm nay
          </span>
        </div>
      </div>
      <HomeIndicator />
        ...
        <div style={{ marginTop: 24 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 4 }}>
            ...
            <div key={i} style={{ padding: '10px 6px', textAlign: 'center', background: p.hide ? 'rgba(154,124,34,0.1)' : 'transparent', border: `1px solid ${p.hide ? CT.goldDeep : CT.hairline2}` }}>
                <Mono style={{ color: CT.muted, fontSize: 10 }}>{p.l}</Mono>
                ...
                <Mono style={{ color: CT.muted, fontSize: 10, marginTop: 6, display: 'block' }}>{p.ng}</Mono>
              </div>
            )}
          </div>
        </div>
        ...
        <section style={{ marginTop: 22 }}>
          ...
          <div style={{ marginTop: 14, display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 4 }}>
            ...
            <div key={i} style={{ padding: '8px 4px', textAlign: 'center', background: hi ? 'rgba(154,124,34,0.08)' : 'transparent', border: `1px solid ${hi ? CT.goldDeep : CT.hairline2}` }}>
                ...
                <div style={{ marginTop: 3, fontFamily: 'var(--mono)', fontSize: 10, color: CT.muted, letterSpacing: '0.04em' }}>{ng}</div>
              </div>
            )}
          </div>
          ...
          <div style={{ marginTop: 14 }}>
            ...
            <div key={i} style={{ display: 'flex', gap: 14, padding: '12px 0', borderBottom: i < arr.length - 1 ? `1px solid ${CT.hairline2}` : 'none', alignItems: 'baseline' }}>
                <div style={{ minWidth: 78 }}>
                  ...
                  <Mono style={{ color: CT.goldDeep, fontSize: 10, marginTop: 3 }}>{s.v}</Mono>
                </div>
          ...
          <div style={{ marginTop: 14 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 3, height: 56 }}>
              ...
              <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', height: '100%', justifyContent: 'flex-end' }}>
                  ...
                  <Mono style={{ color: CT.muted, fontSize: 9.5, marginTop: 3 }}>{i + 1}</Mono>
                </div>
              )}
            </div>
          </div>
            ...
            <div style={{ padding: '10px 12px', background: '#fff', border: `1px solid ${CT.hairline}` }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 3 }}>
                ...
                <div key={i} style={{ padding: '6px 4px', textAlign: 'center', background: hi ? 'rgba(154,124,34,0.06)' : 'transparent', border: `1px solid ${CT.hairline2}` }}>
                    <div style={{ fontFamily: 'var(--mono)', fontSize: 10, color: CT.muted, letterSpacing: '0.04em', textTransform: 'uppercase' }}>{dir}</div>
